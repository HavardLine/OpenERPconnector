...
class MergePDF:
  def __init__(self, paths, outPath):
    print 'mergie text'
...
def searchFolder(folderlist):
    ...
    print glob.glob(expanduser("~")+folderlist+'\*.pdf')
...
if __name__=="__main__":
  ...
  folderlist = [r"\server\LTS\bedrift\ABACUS_arbeidsmappe\faktura_til_betaling",
                  r"\server\LTS\bedrift\ABACUS_arbeidsmappe\bilag",
                  r"\server\LTS\bedrift\ABACUS_arbeidsmappe\loenn"]

  #Calling functions for search and write PDFs
  searchFolder(folderlist[0])
  #writePDF('ABACUS PDF')
  #MergePDF(sourcePaths, outPath)
